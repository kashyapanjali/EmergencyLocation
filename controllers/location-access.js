...
// Define activeConnections for WebSocket clients
...
exports.generateToken = async (req, res) => {
	...
	try {
		...
		// Save the token in the database using PostgreSQL placeholders and upsert
		const tokenQuery = `
    ...
    `;
		...
		// Save the user's location in the database with upsert using PostgreSQL syntax
		const locationQuery = `
...
exports.getLocationByToken = async (req, res) => {
	...
	try {
		const tokenQuery = `
      SELECT userid FROM tokens WHERE token = $1 AND expires_at > NOW()
    `;
		...
		const locationQuery = `
      SELECT latitude, longitude FROM userslocation 
      WHERE userid = $1 
